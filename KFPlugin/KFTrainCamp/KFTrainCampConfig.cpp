...
namespace KFrame
{
    ...
    void KFTrainCampConfig::LoadAllComplete()
    {
        for ( auto& iter : _settings._objects )
        {
            ...
            if ( !kfsetting->_consume_str.empty() )
            {
                ...
                {
                    __LOG_ERROR__( "id=[{}] traincamp config consume is error", kfsetting->_id );
                }
            }
            ...
            if ( !kfsetting->_onekey_consume_str.empty() )
            {
                ...
                {
                    __LOG_ERROR__( "id=[{}] traincamp config onekey consume is error", kfsetting->_id );
                }
            }
        }
    }
