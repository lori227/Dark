...
namespace KFrame
{
    ...
    void KFBuildConfig::ReadSetting( KFNode& xmlnode, KFBuildSetting* kfsetting )
    {
        ...
        kflevelsetting->_consume._str_parse = xmlnode.GetString( "Consume", true );
        kflevelsetting->_onekey_consume._str_parse = xmlnode.GetString( "OnekeyConsume", true );
    }
    ...
    void KFBuildConfig::LoadAllComplete()
    {
        for ( auto& iter : _settings._objects )
        {
            ...
            for ( auto& leveliter : kfsetting->_build_level_settings._objects )
            {
                ...
                KFElementConfig::Instance()->ParseElement( kflevelsetting->_consume, __FILE__, kfsetting->_id );
                KFElementConfig::Instance()->ParseElement( kflevelsetting->_onekey_consume, __FILE__, kfsetting->_id );
            }
        }
    }
}